...
from dc_utils import get_dataset, get_network, get_eval_pool, evaluate_synset, get_time, epoch, DiffAugment, ParamDiffAug, parser_bool, number_sign_augment, downscale
import torchnet
import torch.nn.functional as F
import pickle
import torchattacks
from utils import total_variation, clip_function, negative_log_likelihood, visualize_tsne, negative_log_likelihood_Improved, total_variation_2
best_acc = 0
            ...
            if it in eval_it_pool[1:]:
                for model_eval in model_eval_pool:
                    ...
                    for it_eval in range(args.num_eval):
                        ...
                        _, acc_train, acc_test = evaluate_synset(it_eval, net_eval, image_syn_eval, label_syn_eval, testloader, args,Train_PGD=False, Test_PGD=True)
                    ...
                    best_synset_filename = './checkpoints/ROME_{}_ipc_{}_aug_{}_model_{}_acc_{}.pkl'.format(args.dataset, args.ipc, args.aug, model_eval, np.mean(accs))
                ...
                for net_index, test_model in enumerate(net_list):
                    loss_test, acc_test = epoch('test', testloader, test_model, None, criterion, args, aug = False, PGD=False)
            ...
            for _ in range(args.outer_loop):
                ...
                for j in range(min(args.fetch_net_num, len(shuffled_net_index))):
                    ...
                    for i in range(args.model_train_steps):
                        ...
                        atk_train = torchattacks.PGD(net_train, eps=8/255, alpha=2/225, steps=10, random_start=True)
                        atk_train.set_mode_targeted_by_label(quiet=True)
                        img_resl_atk = atk_train(img_real_, (lab_real_+1)%10)

                        real_logit = net_train(img_resl_atk)
