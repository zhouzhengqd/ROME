...
def total_variation(p,q):
         ...
         return 0.5 * torch.sum(torch.abs(torch.mean(p,dim=0) - torch.mean(q,dim=0)))

def total_variation_lw(p,q):
        
         return 0.5 * torch.sum(torch.abs(torch.mean(p/p.sum(),dim=1) - torch.mean(q/q.sum(),dim=1)))

def total_variation_2(p,q):
        
         return (0.5 * torch.sum(torch.abs(torch.mean(p,dim=0) - torch.mean(q,dim=0))))**2

def neg_log_two_times_one_minus_tv(p, q):
        tv = total_variation(p, q)
        one_minus_tv = 1 - tv
        two_times_one_minus_tv = 2 * one_minus_tv
        neg_log = -torch.log(two_times_one_minus_tv)
        return neg_log

def clip_function(x,y):
    x_unorm = (torch.mean(x,dim=0) - torch.mean(y,dim=0)) / torch.std(y,dim=0)  
    dist_clip = torch.sum(torch.mean(torch.square(x_unorm - torch.clamp(x_unorm, 0.0, 1.0)), dim=0))
    return dist_clip

def gaussian_pdf(x, y):
    sigma = torch.exp(torch.std(y,dim=0))
    return 1 / (torch.sqrt(2 * torch.tensor(math.pi)) * sigma) * torch.exp(-0.5 * ((torch.mean(x,dim=0) - torch.mean(y,dim=0)) / sigma)**2)

def gaussian_pdf_Improved(x, y):
    sigma = torch.exp(torch.std(y,dim=0))
    return 1 / (torch.sqrt(2 * torch.tensor(math.pi)) * sigma) * torch.exp(-0.5 * ((x.view(-1,y.size(1)).mean(dim=0) - y.mean(dim=0)) / sigma)**2)

def negative_log_likelihood(x, y):
    return -torch.sum(torch.log(gaussian_pdf(x, y)))

def negative_log_likelihood_Improved(x, y):
    return -torch.sum(torch.log(gaussian_pdf_Improved(x, y)))

def get_default_convnet_setting():
    net_width, net_depth, net_act, net_norm, net_pooling = 128, 3, 'relu', 'instancenorm', 'avgpooling'
    return net_width, net_depth, net_act, net_norm, net_pooling

def visualize_tsne(intermediate_outputs, labels, num_classes,save_path):
    intermediate_outputs_cpu = intermediate_outputs.detach().cpu().numpy()

    # 使用 t-SNE 进行降维
    tsne = TSNE(n_components=2, random_state=42)
    intermediate_outputs_tsne = tsne.fit_transform(intermediate_outputs_cpu)
    plt.figure(figsize=(10, 8))
    for class_label in range(num_classes):
        indices = labels.detach().cpu().numpy() == class_label
        plt.scatter(intermediate_outputs_tsne[indices, 0], intermediate_outputs_tsne[indices, 1], label=f'Class {class_label}')

    plt.title('中间层输出的t-SNE可视化')
    plt.xlabel('t-SNE维度1')
    plt.ylabel('t-SNE维度2')
    plt.legend()
