...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, step=-1, bsr=False, pooling=False, partial=None, PGD=False):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, upsample=False, pooling=False, partial=None, aug=True, tqdm_bar=False, Train_PGD=False, Test_PGD=False):
    net = net.to(args.device)
    ...
    for ep in training_range:
        if not upsample:
            loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug = aug, pooling=pooling,PGD=Train_PGD)
        else:
            loss_train, acc_train = epoch_upsample('train', trainloader, net, optimizer, criterion, args, aug = aug,PGD=Train_PGD)
        ...
        if tqdm_bar and 'imagenet_subset' not in args.dataset:
            ...
            loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False, partial=partial, PGD=Test_PGD)
